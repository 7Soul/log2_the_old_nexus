...
import "mod_assets/scripts/spells/red_aoe.lua" -- used by monsters casting spells
...
defineObject{
		...
		onDrawGui = function(self, context)	
			...
			if context.keyDown("O") then
				...
				if context.keyDown("X") then
					functions.script.move(0.01*multi,0,0)
				end
				if context.keyDown("C") then
					functions.script.move(0,0.01*multi,0)
				end
				if context.keyDown("Z") then
					functions.script.move(0,0,0.01*multi)
				end
			end
			...
			if context.keyDown("P") then
				...
				context.drawText(text, 	w - (f2 * (w / 2  + context.getTextWidth(text))), h - (f2 * h / 2))
				if context.keyDown("X") then
					functions.script.rotate(0.1*multi,0,0)
				end
				if context.keyDown("C") then
					functions.script.rotate(0,0.1*multi,0)
				end
				if context.keyDown("Z") then
					functions.script.rotate(0,0,0.1*multi)
				end
			end
			...
			if context.keyDown("M") then
				functions.script.revealMap(false)
				print("Map revealed")
				functions.script.keypressDelaySet(20)
			end

			if context.keyDown("K") then
				-- Ctrl+ Shift + K --> Kill all mobs in map
				if context.keyDown("ctrl") then
					functions.script.killAllMobs()
					print("Mobs killed")
					functions.script.keypressDelaySet(20)
				else
	...
	},
	{
		...
		onActivate = function(self)
			self.go.gametime:increment()
			local t = GameMode.getTimeOfDay()
			local v = party.gametime:getValue()
			if v > 10 and v % 2 == 0 then
				functions.script.updateSky(t) -- updates sky and tides
